...
namespace jirachi_core {
    public class Gen1SaveFileHandler : ISaveFileHandler {
        ...
        public static PokemonModel ReadPokemonFromBytes(byte[] bytes, PokemonLocation location, int index) {
            ...
            // national dex id
            nationalDexNumber = Gen1PokemonByteFunctions.ReadNationalDexNumberFromPkmnBytes(pokemonDataBytes);

            // current HP
            currentHP = Gen1PokemonByteFunctions.ReadCurrentHPFromPkmnBytes(pokemonDataBytes);

            // level
            level = Gen1PokemonByteFunctions.ReadLevelFromPkmnBytes(pokemonDataBytes);
